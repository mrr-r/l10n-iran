...
class ResConfigSetting(models.TransientModel):
    ...
    sms_line_number = fields.Char("SMS line number")
    sms_user_api_key = fields.Char("UserApiKey")
    sms_secret_key = fields.Char("SecretKey")
    twilio_overwrite_odoo_sms = fields.Boolean("Rewrite the messaging service")
    ...
    def set_values(self):
        ...
        param_obj.sudo().set_param(
            "ql_scheduler_reminder.sms_line_number", self.sms_line_number
        )
        param_obj.sudo().set_param(
            "ql_scheduler_reminder.sms_user_api_key", self.sms_user_api_key
        )
        param_obj.sudo().set_param(
            "ql_scheduler_reminder.sms_secret_key", self.sms_secret_key
        )
        param_obj.sudo().set_param(
            "ql_scheduler_reminder.twilio_overrwrite_odoo_sms",
