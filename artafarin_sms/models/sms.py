...
class SMS(models.Model):
    ...
    def send_sms(self, delete_all=False, raise_exception=False):
        ...
        response = requests.post(urlGetToken, json=body_getToken, headers=header_getToken)
    ...
    def send_sms(self,token, phone_number, message, line_number):
        header_sendSms = {'Content-Type': 'application/json', 'x-sms-ir-secure-token': '{}'.format(token)}
        
        body_sendSms = {
            "ParameterArray": [
                {"Parameter": "code", "ParameterValue": "{}".format(message)},
            ],
